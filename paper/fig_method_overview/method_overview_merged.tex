...
\node[block, fill=cCollect!70, right=1.1cm of train, text width=4.9cm] (collect) {Roll out $\pi_\theta$;\\expert takes over on\\predicted failure};
...
\node[inner sep=0, anchor=north west] (panels) at ($(rowy)+(2.9,1.55)$) {\includegraphics[width=16.6cm]{calibration_panels.pdf}};
\begin{scope}[on background layer]
  \node[region, draw=cCal!60, fill=cCal!6, fit=(panels), inner sep=6pt] (calbox) {};
\end{scope}
\node[lab, text=cCal!70!black, anchor=south west] at ($(calbox.north west)+(0.15,0.02)$) {\textbf{Policy calibration + augmentation} \quad offline, no expert required};
