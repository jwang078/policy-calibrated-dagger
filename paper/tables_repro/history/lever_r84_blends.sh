...
# finished recording it (its stats sidecar exists). Then LEVER_R84_BLENDS_DONE.
set -u
...
CKPT=$LR/outputs/training/diffusion_approach_lever_13_smooth_r${RES}_delta_basewristng/checkpoints/last/pretrained_model
log() { echo "[r84blend $(date +%H:%M:%S)] $*"; }
until [ -f "$S/LEVER_R84_BC_DONE" ]; do sleep 30; done
INT=""; until [ -n "$INT" ]; do INT=$(ls -d $C/lever_d100_03dagcap_r${RES}*_r_dag1 2>/dev/null | head -1); [ -n "$INT" ] && [ -f "$LR/outputs/dataset_stats/$(basename $INT)/stats_rel64.json" ] || { INT=""; sleep 60; }; done
SHORT=$(basename $INT); log "intervention dataset: $SHORT"; echo "$SHORT" > $S/LEVER_R84_INT_SHORT
EPS=$(python3 -c "
import json
...
lane() {  # PORT ratios...
  ...
  python -u scripts/launch_nodes.py --robot sim_ur_pybullet_small_engine_new_interactive --robot_port $PORT --hostname 127.0.0.1 \
    ...
    --strict_goal_tolerances > "$S/r${RES}_blend_sim_$PORT.log" 2>&1 &
  local SIM=$!
  ...
  for R in "$@"; do
    ...
    nice -n 10 python my_scripts/augment_dataset_with_blending.py \
      ...
      --blend_ratio_backoff=false --blend_tube_steps=0 --max_tube_breach_ratio=0 > "$S/r${RES}_blend_$TAG.log" 2>&1
    log "$TGT rc=$?"
  done
  kill $SIM 2>/dev/null
}
...
wait $P1 $P2
touch "$S/LEVER_R84_BLENDS_DONE"; log "LEVER_R84_BLENDS_DONE"
