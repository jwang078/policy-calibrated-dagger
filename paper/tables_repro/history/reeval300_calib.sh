...
# arm in the calibrated-noise grids, all three seeds. Own sim lane: port 6031.
set -u
...
for i in $(seq 1 120); do grep -q "Serving\|serve loop\|Listening\|Robot server" "$S/recal_sim.log" 2>/dev/null && break; sleep 2; done
...
for SD in scarcity_study scarcity_study_s2; do
  ...
  for K in 3 4 5; do eval_one $SD q${K}_dnfloor || true; done
done
log "RECAL_DONE"
