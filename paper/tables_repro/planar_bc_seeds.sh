...
log() { echo "[bcseed$LANE $(date +%H:%M:%S)] $*"; }
SUBSET100=$(python3 -c "print('['+','.join(str(i) for i in range(100))+']')")
cd /home/jennyw2/code/SplatSim
python -u scripts/launch_nodes.py --robot sim_pybullet_planar_interactive --robot_port $PORT --hostname 127.0.0.1 --eval_benchmark_repo_id JennyWWW/eval_planar_3joint_benchmark --robot_name planar_3joint --headless --control_gui > "$S/bcseed${LANE}_sim.log" 2>&1 & SIM=$!; trap 'kill ${SIM:-} 2>/dev/null' EXIT
for i in $(seq 1 120); do grep -q "Serving\|serve loop\|Listening\|Robot server" "$S/bcseed${LANE}_sim.log" 2>/dev/null && break; sleep 2; done; log "sim up on $PORT"; cd "$LR"
eval_one() { local GD=$1 NAME=$2 SD=$3; local CK="/home/jennyw2/code/lerobot/outputs/training/bc175k/checkpoints/last/pretrained_model"; local D="$LR/outputs/eval300/$GD/${NAME}_e$SD"
...
JOBS=(); for SD in 1 2; do JOBS+=("base bc175k $SD"); done
n=0; for J in "${JOBS[@]}"; do [ $((n % NL)) = $LANE ] && eval_one $J; n=$((n+1)); done
log "BCSEED_LANE${LANE}_DONE"
