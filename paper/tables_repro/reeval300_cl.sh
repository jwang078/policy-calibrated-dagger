...
python -u scripts/launch_nodes.py --robot sim_pybullet_planar_interactive \
  ...
  > "$S/recalcl_sim.log" 2>&1 &
SIM=$!
trap 'kill ${SIM:-} 2>/dev/null' EXIT
for i in $(seq 1 120); do grep -q "Serving\|serve loop\|Listening\|Robot server" "$S/recalcl_sim.log" 2>/dev/null && break; sleep 2; done
...
for K in 1 2 3 4 5; do
  eval_one scarcity_study_cl q${K}_dnpoolcl || true
done
log "RECALCL_DONE"
