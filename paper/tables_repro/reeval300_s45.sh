...
# arm in the calibrated-noise grids, all three seeds. Own sim lane: port 6027.
set -u
...
python -u scripts/launch_nodes.py --robot sim_pybullet_planar_interactive \
  --robot_port 6027 --hostname 127.0.0.1 \
...
for i in $(seq 1 120); do grep -q "Serving\|serve loop\|Listening\|Robot server" "$S/re45_sim.log" 2>/dev/null && break; sleep 2; done
log "sim node up on 6027"
cd "$LR"
...
eval_one() {
  ...
  $EV \
    ...
    --env.external_port=6027 --env.max_parallel_tasks=1 \
...
for K in 1 2 3 4 5; do
  for SD in scarcity_study_s4 scarcity_study_s5; do eval_one $SD q$K || true; done
done
for SD in scarcity_study_s4 scarcity_study_s5; do
  for A in $ISO; do eval_one $SD $A || true; done
done
log "RE45_DONE"
