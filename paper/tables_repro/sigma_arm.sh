...
# Runs BEFORE the seeds-4/5 chain (which now gates on $S/aniso_done).
set -u
...
log() { echo "[sigarm $(date +%H:%M:%S)] $*"; }
until [ -f $S/analysis/sigma_deltas_s2_dag2.npz ]; do sleep 120; done
$PY $S/analysis/build_sigma_schedule.py s1 05dag
$PY $S/analysis/build_sigma_schedule.py s2 06dag
while pgrep -f "bin/lerobot-trai[n]" >/dev/null; do sleep 300; done
log "training sigma-alpha arms"
cd "$LR"
run() {
  local SFX=$1 TAG=$2
  local DIR="$LR/outputs/training/scarcity_study/q2_dnsig"
  [ "$SFX" = "s2" ] && DIR="$LR/outputs/training/scarcity_study_s2/q2_dnsig"
  ...
  bash my_scripts/resume_training.sh "$BASE_CKPT" \
    ...
    --dataset.dart_state_noise_schedule=$S/analysis/noise_schedule_sigma_alpha_${SFX}_K2.json \
    --dataset.dart_state_noise_scale=1.0 \
    --dataset.dart_vel_noise_std=0.3 \
    $SW \
    --steps=175000 --eval_freq=175000 --save_freq=175000 \
    --output_dir="$DIR" --job_name="sig_${SFX}_q2_dnsig" \
    --policy.repo_id="sig_${SFX}_q2_dnsig" --policy.push_to_hub=false \
    --eval.n_episodes=100 \
    > "$DIR.log" 2>&1
  log "$SFX rc=$?"
}
