...
class SharedAutonomyPolicyWrapper(PreTrainedPolicy):
    ...
    def __init__(
        ...
        # Tube re-blend budget (see observation_teleop_source blend
        # finalize): max commanded offset from the guidance chunk, in units
        # of the guidance chunk's own median step. When exceeded, the blend
        # is RE-RUN at a reduced ratio (never edited in action space — the
        # emitted chunk is always a genuine policy-blend product). 0 disables.
        self.blend_tube_steps: float = 8.0
