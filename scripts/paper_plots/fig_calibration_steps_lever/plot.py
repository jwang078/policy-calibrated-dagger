...
# ── CONFIG ───────────────────────────────────────────────────────────────────
EPISODE = 2  # source intervention episode (r_dag1, even index)
...
SUPTITLE = "Offline policy calibration of the injected noise (lever, round 1)"  # "" = none
FIGSIZE = (12.6, 3.6)
DPI = 150
# ─────────────────────────────────────────────────────────────────────────────
...
z = np.load(os.path.join(HERE, "sigma_deltas_lever_q1_dag1.npz"))
d = z[f"ep{EPISODE}_d"]
...
row = np.asarray(
    json.load(open(os.path.join(HERE, "noise_schedule_pooled_lever_K1.json")))[f"JennyWWW/{SRC_REPO}"][
        str(EPISODE)
    ][0],
    dtype=float,
)
...
for (i, j), v in zip(IU6, row):
    Sg[i, j] = Sg[j, i] = v
Sg *= ms**2
_al = json.load(open(os.path.join(HERE, "dart_scale_lever_K1.json")))
