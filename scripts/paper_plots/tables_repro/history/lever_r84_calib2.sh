#!/bin/bash
# r84 round 2 calibrated arm: W from rounds 1+2 blends -> Sigma-hat with the
# round-1 HG policy (+40k) on dag1 and dag2 -> pooled K=2 schedule -> cal r2
# FROM BASE on base + dag1 + dag2 (+40k, same weights as HG r2, DART-wrapped,
# constant lr) -> 100-ep evals of hg2/cal2 at +40k and +20k on two nodes.
set -u
...
HG1=$TR/${BASE}_d100_03dagcap_r${RES}_ft_dag1/checkpoints/last/pretrained_model
...
log() { echo "[r84cal2 $(date +%H:%M:%S)] $*"; }
until [ -f "$S/LEVER_R84_BLENDS2_DONE" ]; do sleep 60; done
cd "$LR"
INT_PREFIX=$PFX $PY $S/analysis/measure_w_lever.py 1,2 > $S/analysis/w_lever_r${RES}_rounds1,2.txt 2>&1
W=$(grep -oE "W = [0-9.]+" "$S/analysis/w_lever_r${RES}_rounds1,2.txt" | head -1 | awk '{print $3}')
log "W(rounds 1+2) = '$W'"; grep -E "dag|W =|CI" "$S/analysis/w_lever_r${RES}_rounds1,2.txt"
[ "$($PY -c "w=float('${W:-nan}'); print('yes' if 3<=w<=40 else 'no')")" = yes ] || { log "ABORT: W out of range"; exit 1; }
for R in 1 2; do D=$([ $R = 1 ] && echo $DAG1 || echo $DAG2); $PY $S/analysis/measure_sigma_lever.py r${RES}_q2_dag$R "$HG1" "$D" > $S/analysis/sigma_r${RES}_q2_dag$R.log 2>&1; log "sigma dag$R rc=$?"; done
INT_PREFIX=$PFX TAGPFX=r${RES}_ $PY $S/analysis/build_pooled_schedule_lever.py 2 $W | tee -a $S/analysis/sigma_r${RES}_q2_dag2.log
SCHED=$S/analysis/noise_schedule_pooled_lever_r${RES}_K2.json; [ -f "$SCHED" ] || { log "ABORT: no K2 schedule"; exit 1; }
until [ -f "$S/LEVER_R84_HG2_DONE" ]; do sleep 60; done     # GPU: wait for HG r2
WTS=$(cat $S/LEVER_R84_WTS2)
...
DIR=$TR/lever_r${RES}_calib/q2_dnpool; mkdir -p $TR/lever_r${RES}_calib
if [ ! -d "$DIR/checkpoints/last" ]; then
  rm -rf "$DIR"; log "training calibrated r2 from base: weights $WTS"
  bash my_scripts/resume_training.sh "$BC" --optimizer.lr=1e-6 --scheduler.name=constant --num_workers=8 \
    ...
    --output_dir="$DIR" --job_name=lever_r${RES}_cal2 --policy.repo_id=lever_r${RES}_cal2 --policy.push_to_hub=false --wandb.enable=false > "$DIR.log" 2>&1
  log "cal2 rc=$?"
fi
# ── evals on two nodes ──
start_node() { cd /home/jennyw2/code/SplatSim; python -u scripts/launch_nodes.py --robot sim_ur_pybullet_small_engine_new_interactive --robot_port $1 --hostname 127.0.0.1 \
    --eval_benchmark_repo_id JennyWWW/eval_splatsim_approach_lever_13_benchmark --headless --control_gui --render_mode splat > "$S/r${RES}_cal2eval_sim_$1.log" 2>&1 & echo $!; }
...
( eval_one r${RES}_cal2 "$DIR/checkpoints/last/pretrained_model" 6043; eval_one r${RES}_cal2_20k "$DIR/checkpoints/095000/pretrained_model" 6043 ) & PA=$!
( eval_one r${RES}_hg2 "$TR/lever_r${RES}_fb/hg2/checkpoints/last/pretrained_model" 6044; eval_one r${RES}_hg2_20k "$TR/lever_r${RES}_fb/hg2/checkpoints/095000/pretrained_model" 6044 ) & PB=$!
wait $PA $PB
touch "$S/LEVER_R84_CAL2_DONE"; log "LEVER_R84_CAL2_DONE"
