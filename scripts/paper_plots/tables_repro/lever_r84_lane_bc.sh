...
JOBS=("bc_r84_e1|/home/jennyw2/code/lerobot/outputs/training/diffusion_approach_lever_13_smooth_r84_delta_basewrist/checkpoints/075000/pretrained_model|1" "bc_r84_e2|/home/jennyw2/code/lerobot/outputs/training/diffusion_approach_lever_13_smooth_r84_delta_basewrist/checkpoints/075000/pretrained_model|2"); if [ "${BC_ONLY:-0}" = 1 ]; then n=0; for J in "${JOBS[@]}"; do IFS="|" read -r NAME CK SD <<< "$J"; eval_one "$NAME" "$CK" "$SD"; done; log "LANE${LANE}_DONE"; exit 0; fi
for K in 1 2; do for PASS in 1 2; do for TS in 0 1 2; do for ARM in cal hg; do
