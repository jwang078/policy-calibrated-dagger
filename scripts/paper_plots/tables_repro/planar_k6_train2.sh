#!/bin/bash
# Planar K=6 training lane v2 (arm-major order: all q6, then all q6_dnpool, then all q6_dnsig across the lane's lineages). usage: planar_k6_train.sh "<lineages e.g. s1 s3 s5>" ; per lineage: q6 (HG from base) ->
...
run_arm() { local SFX=$1 TAG=$2 GD=$3 NAME=$4; shift 4
  ...
  bash my_scripts/resume_training.sh "$BASE_CKPT" --optimizer.lr=${LR_FT:-1e-5} --num_workers=4 --dataset.multi_source_feature_intersection=true --dataset.repo_id= \
...
calib() { local SFX=$1 TAG=$2 GD=$3
  [ -f $S/analysis/noise_schedule_pooled_${SFX}_K6.json ] && return
  ...
  $PY $S/analysis/build_sigma_schedule_k.py $SFX $TAG 6 > $S/k6_sched_$SFX.log 2>&1; log "schedule $SFX rc=$?"; }
DART='--dataset.dart_relabel=true --dataset.dart_self_relabel_pattern=_r_dag\d+$ --dataset.dart_state_noise_std=0 --dataset.dart_state_noise_scale=1.0 --dataset.dart_vel_noise_std=0.3'
if [ "$FIXED" = 0 ]; then
  for SFX in $LINS; do set -- $(lin $SFX); TAG=$1; GD=$2
    ...
    run_arm $SFX $TAG $GD q6 --dataset.dart_relabel=false; calib $SFX $TAG $GD; done
  for V in pool sig; do for SFX in $LINS; do set -- $(lin $SFX); TAG=$1; GD=$2
    SCH=$S/analysis/noise_schedule_$([ $V = pool ] && echo pooled || echo sigma_alpha)_${SFX}_K6.json
    [ -f "$SCH" ] || { log "ABORT $SFX: no K6 schedule"; continue; }
    run_arm $SFX $TAG $GD q6_dn$V $DART --dataset.dart_state_noise_schedule=$SCH $SW; done; done
else
  for STD in ${STDS:-4 2 8 12 16}; do for SFX in $LINS; do set -- $(lin $SFX); TAG=$1; GD=$2
    until [ -f "$LR/outputs/dataset_stats/planar_12_${TAG}_diff_r_dag6/stats_rel64.json" ]; do sleep 60; done
    run_arm $SFX $TAG $GD q6_dn${STD}swv --dataset.dart_relabel=true '--dataset.dart_self_relabel_pattern=_r_dag\d+$' --dataset.dart_state_noise_std=$STD --dataset.dart_vel_noise_std=0.3 $SW; done; done
fi
