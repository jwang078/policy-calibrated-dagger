...
def main() -> None:
    ...
    S_bl, A_bl, S_src = S_bl[:, :n], A_bl[:, :n], S_src[:, :n]
    ...
    fig.suptitle(
        ...
        f"grey=demo  blue=executed states  red=stored action (label track)  arrows: state → label"
    )
    ...
    for k in range(n_planes):
        ...
        idx = np.arange(0, len(S_bl), max(1, args.arrow_every))
        ax.quiver(
            S_bl[idx, jx],
            S_bl[idx, jy],
            (A_bl - S_bl)[idx, jx],
